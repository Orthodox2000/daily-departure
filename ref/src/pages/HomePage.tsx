...
import { MapPin, Calendar, Users, Search, BadgePercent, ShieldCheck, Globe, Headphones, Star, ArrowRight, PlaneTakeoff, Sparkles, CheckCircle2, ChevronLeft, ChevronRight } from 'lucide-react';
...
export const HomePage: React.FC<HomePageProps> = ({
  ...
  return (
    <div className="min-h-screen bg-[#FAF9F6]">
      {/* Hero Section */}
      <section className="relative min-h-[520px] sm:min-h-[580px] flex items-center justify-center text-white pb-24 sm:pb-28">
        ...
        <div className="absolute inset-0 overflow-hidden pointer-events-none">
          {heroBackgrounds.map((bg, idx) => (
            <img 
              ...
              className={`absolute inset-0 w-full h-full object-cover object-center brightness-[0.6] transition-opacity duration-1000 ease-in-out transform scale-105 ${
                idx === activeHeroIdx ? 'opacity-100' : 'opacity-0'
              }`}
            />
          ))}
            ...
            {TESTIMONIALS.map((t, idx) => (
              <div key={idx} className="bg-[#FAF9F6] p-8 rounded-sm border border-[#e5e5e5] flex flex-col justify-between">
                ...
                <div className="flex items-center gap-3 pt-4 border-t border-[#e5e5e5]">
                  <img src={t.avatar} alt={t.name} className="w-11 h-11 rounded-full object-cover border border-brand-maroon" />
                  <div>
